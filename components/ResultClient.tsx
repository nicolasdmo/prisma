...
function ViralShare({ archetype }: { archetype: Archetype }) {
  ...
  const waText  = encodeURIComponent(
    `Hice el test PRISMA y soy ${archetype.name} (${archetype.code}) — ${archetype.tagline}. ¿Y vos qué sos? 👇\n${testUrl}`
  );
      ...
      <div className="px-6 py-8 flex flex-col items-center gap-5 text-center">
        ...
        <div>
          ...
          <p className="font-serif text-2xl text-ink leading-snug mb-2" style={{ fontFamily: 'var(--font-serif)' }}>
            Retá a alguien que crea<br />que ya te conoce.
          </p>
          <p className="text-ink-soft text-sm max-w-xs mx-auto leading-relaxed">
            Mandales el test — capaz se sorprenden con lo que descubren de sí mismos.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full max-w-sm">
          ...
          <a
            ...
            className="flex-1 flex items-center justify-center gap-2.5 py-3.5 px-5 rounded-pill font-mono text-xs tracking-widest uppercase text-white hover:opacity-90 active:scale-95 transition-all"
          ...
          <button
            onClick={handleCopy}
            className="flex-1 flex items-center justify-center gap-2.5 py-3.5 px-5 rounded-pill font-mono text-xs tracking-widest uppercase border transition-all active:scale-95"
            style={{
              borderColor: copied ? `${archetype.color}60` : 'var(--line)',
              background:   copied ? `${archetype.color}10` : 'var(--bg-card)',
            ...
            {copied ? (
              ...
              <>
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                  ...
                  <path d="M4 4V2.5A1.5 1.5 0 015.5 1H11.5A1.5 1.5 0 0113 2.5V8.5A1.5 1.5 0 0111.5 10H10" stroke="currentColor" strokeWidth="1.4"/>
                </svg>
                Copiar link
              </>
            )}
